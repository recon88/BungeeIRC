...
public class IRCMain extends Plugin implements Listener {
	...
	public String replaceMinecraftPlaceholders(ProxiedPlayer player, String message, String format) {
		...
		return format.replaceAll("%from", player.getServer().getInfo().getName()).replaceAll("%uuid", player.getUniqueId().toString()) // for people who really want UUIDs in chat
		.replaceAll("%player", player.getName()).replaceAll("%message", message).replaceAll("%displayname", player.getDisplayName());
	}

	public String replaceIrcPlaceholders(String player, String channel, String message, String format) {

		return format.replaceAll("%from", channel).replaceAll("%uuid", "") // for people who really want UUIDs in chat
		.replaceAll("%player", player).replaceAll("%message", message).replaceAll("%displayname", player);
	}
